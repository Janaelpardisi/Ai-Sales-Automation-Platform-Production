...
async def add_unsubscribe_token_column():
    ...
    print("🔧 Starting migration: Add unsubscribe_token to leads")
    ...
    async with engine.begin() as conn:
        ...
        if row and row[0] > 0:
            print("✅ Column 'unsubscribe_token' already exists, skipping...")
            return
        ...
        print("🔑 Generating tokens for existing leads...")
        ...
        print(f"✅ Generated tokens for {len(lead_ids)} existing leads")
        
        # Create unique index
        print("📊 Creating unique index on unsubscribe_token...")
        await conn.execute(text("""
